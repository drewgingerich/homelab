{ inputs, ... }:
{
  flake.homeModules.autorestic =
    ...
    {
      options.custom.autorestic.enable = lib.mkEnableOption "Autorestic backups";
  ...
  flake.nixosModules.autorestic =
    { ... }:
    {
      home-manager.sharedModules = [ inputs.self.homeModules.autorestic];
    };
}
